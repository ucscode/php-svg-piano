...
class PHPSVGPiano
{
    ...
    private function set_black_keys($octave, $position, $group)
    {
        ...
        foreach ($position as $key => $white_key_axis) {
            ...
            if (!empty($octave_notes)) {
                ...
                if (in_array($sharp, $octave_notes['notes'])) {
                    $note = $sharp;
                } elseif (in_array($flat, $octave_notes['notes'])) {
                    $note = $flat;
                }
                if ($note) {
    ...
    private function play(?string $notes)
    {
        ...
        foreach ($notes as $key => $note) {
            ...
            if (!is_numeric(substr($note, -1))) {
                $notes[$key] = $note = ($note . "4");
            }
            ...
            if (!isset($octave_notes[$octave])) {
                $octave_notes[$octave] = array();
            }
